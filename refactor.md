...
**后端现有模块里，哪些适合保留主体逻辑，哪些适合局部重构，哪些应该只保留需求而重写结构。**

如果不先做这层分类，后续很容易出现两种错误：

1. 本来可以保留的模块被过度重写。
2. 本来应该重构的模块继续被打补丁，越拖越难收口。

因此，后端模块不建议只分“保留 / 重写”两类，而建议分成下面四类。

### 15.1 四类分类标准

#### A 类：保留主体逻辑，优先做适配层

这类模块的核心能力已经成立，问题主要在接口、数据边界、错误语义或调用方式。

处理原则：

- 不急着重写内部实现。
- 先通过 Application API 包起来。
- 优先统一 DTO、错误语义、输入输出边界。

#### B 类：保留能力，但要局部重构

这类模块的能力有价值，但内部结构已经开始阻碍后续扩展。

处理原则：

- 保留已有能力和规则。
- 按职责拆分局部结构。
- 接受模块内部会发生明显重组，但不整体推倒。
...
| 模块                        | 分类     | 当前判断                                       | 当前处理建议                      | 建议阶段         |
| --------------------------- | -------- | ---------------------------------------------- | --------------------------------- | ---------------- |
| `core/pipeline`           | A + B    | 流水线骨架已经成立，但字幕相关边界还不够清晰   | 保留执行骨架，字幕链路局部重构    | 现在持续推进     |
| `core/resources`          | A        | 模型注册、下载、状态、校验方向正确             | 继续保留主体逻辑，通过 app 层暴露 | 现在持续推进     |
| `core/asr`                | A        | 识别能力本身成立，主要缺统一 DTO 和服务入口    | 不重写识别逻辑，先纳入应用服务    | Phase 2 / 3      |
| `core/translate`          | A        | 翻译能力较完整，缓存与字幕结构边界还可再收束   | 保留主体逻辑，后续统一数据边界    | Phase 3          |
| `core/vocal_separator`    | A        | 核心能力清晰，问题不在算法而在资源入口统一     | 保留实现，继续通过 API 层接入     | Phase 2          |
| `core/tts`                | B        | 能力丰富，但运行期、音色、预处理、配置耦合较多 | 保留能力，逐步拆职责              | Phase 3 / 4      |
| `core/script_to_subtitle` | B        | 功能有价值，但和主流水线仍偏并行支线           | 保留能力，逐步并入统一字幕模型    | Phase 3          |
| `mixer` / 混音逻辑        | B        | 能用，但模块边界还不够清楚                     | 保留现有逻辑，后续按域模型再抽    | Phase 4          |
| `src/app`                 | 新核心层 | 已经开始成型，是未来主线中心                   | 持续扩展为正式应用服务层          | 现在持续推进     |
| CLI 旧命令结构              | C        | 历史上按功能直连 core，不适合作为长期模板      | 只保留需求，用新分组式 CLI 重建   | 现在持续推进     |
| 旧 GUI 调后端方式           | C        | 能用，但强耦合，不适合作为未来架构模板         | 只保留需求，不保留当前组织结构    | 后续前端阶段     |
| `setup.ps1`               | C / D    | 当前仍有现实价值，但不应继续承载核心能力       | 冻结其中心地位，逐步退场          | 后续资源层稳定后 |
| 零散辅助脚本                | D        | 有些仍有用，但不是主线                         | 暂时冻结，避免继续扩展            | 后续再判断       |

### 15.3 这张分类图的实际用途

后续每次准备动一个模块前，都先问三个问题：

1. 这个模块属于 A、B、C、D 哪一类？
2. 这次是要保留主体逻辑，还是只保留需求？
3. 这次改动是在服务 Application API 主线，还是在旁路加复杂度？

如果这三个问题答不清，最好不要直接开改。

### 15.4 当前主线对应的模块优先级

基于现在的分类，当前主线最适合优先推进的是：

1. `src/app`
   继续把应用服务层做实。
2. `core/pipeline`
   重点是把字幕数据边界逐步接到 `SubtitleService`。
3. `core/resources`
   继续稳定模型与资源能力的后端内聚。
4. `core/asr` / `core/translate` / `core/tts`
   逐步通过应用服务接入，而不是直接暴露给 CLI / GUI / 未来 Web。
...
当前后端最重要的判断不是“全都要不要重写”，而是：

- `pipeline`、`resources`、`asr`、`translate`、`vocal_separator` 这些，大多属于“保留主体逻辑，先收口边界”。
- `tts`、`script_to_subtitle`、混音相关更像“保留能力，但要局部重构”。
- CLI 旧结构、GUI 旧调用方式属于“只保留需求，不保留现有结构”。
- `setup.ps1` 和零散脚本属于“冻结并逐步退场”的候选。

这意味着后续重构不该是“全面推倒”，而应该是：

**先建立统一应用层，再按分类逐步收编现有后端资产。**
