...
执行入口也完成去重：仓库内没有调用方的 `/pipeline-runs/start`、`/pipeline-runs/execute` 与同步 `POST /tool-runs` 已删除，并增加负向路由守卫。Pipeline 与 Tool 现在都只有“创建即提交”的正式 HTTP 执行入口；服务层同步方法仅供内部组合与测试使用。
