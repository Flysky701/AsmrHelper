...
methods_to_remove = ['preview_voice', '_on_preview_finished']
    ...
    if isinstance(node, ast.ClassDef) and node.name == 'MainWindow':
        for n in node.body:
            if isinstance(n, ast.FunctionDef) and n.name in methods_to_remove:
                ...
                methods_code.append(''.join(lines[start:end]))
...
with open('preview_tmp.py', 'w', encoding='utf-8') as f:
    f.write(''.join(methods_code))

print("Extracted preview methods")
